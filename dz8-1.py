...
# регулярного выражения извлекает имя пользователя и почтовый домен из email адреса и возвращает их в виде словаря.
...
def email_parse(email_address):
    ...
    if not re_email.match(email_address):
        raise ValueError(f'wrong e-mail: {email_address}')
    print(re_email.match(email_address).groupdict())

for i in ['someone@geekbrains.ru', 'gerasim&mumu@ru', 'alexP-ravdiNN@olson.ru']:
    try:
        email_parse(i)
